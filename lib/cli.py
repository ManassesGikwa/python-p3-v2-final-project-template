...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            query = input("Enter vehicle ID, model, make, or color: ")
            search_vehicle(query)
        elif choice == '3':
            vehicle_ids = input("Enter the Vehicle IDs of the vehicles you want to buy (separated by commas): ").split(',')
            payment_method = input("Choose payment method (1 for cash/2 for hirepurchase): ")
            make_purchase(vehicle_ids, payment_method)
        elif choice == '0':
            print("Goodbye!")
            break
        else:
